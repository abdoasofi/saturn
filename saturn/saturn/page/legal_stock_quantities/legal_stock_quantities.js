...
// --- ON PAGE LOAD ---
frappe.pages['legal-stock-quantities'].on_page_load = function(wrapper) {
    ...
    let page = frappe.ui.make_app_page({ parent: wrapper, title: __("Legal Stock Quantities"), single_column: true });
    $(page.main).html(`<div class="text-center p-5"><h4>${__("Initializing Viewer...")}</h4></div>`);

    loadScripts([QRCODE_GEN_URL, QRCODE_SCAN_URL])
        .then(() => {
            frappe.call({ 
                method: "saturn.saturn.page.legal_stock_quantities.legal_stock_quantities.get_company_info", 
                callback: function(r) { 
            ...
            <div id="item-details-section" style="display: none;">
                <div class="item-details-grid">
                    <div class="main-column">
                        <div class="item-main-details page-card">
                            ...
                            <dl class="row">
                                ...
                                <dt class="col-sm-4">${__("EN-DETAIL SHOWROOM")}</dt><dd class="col-sm-8"><strong><span id="item-price_1"></span></strong></dd>
                                <dt class="col-sm-4">${__("EN-GROSS")}</dt><dd class="col-sm-8"><strong><span id="item-price_2"></span></strong></dd>
                                <dt class="col-sm-4">${__("Description")}</dt><dd class="col-sm-8" id="item-description"></dd>
                            </dl>
                        </div>
                        <div class="stock-levels-card page-card">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tbody id="stock-levels-table-body"></tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                    <div class="item-side-details page-card">
                        <div class="item-image-wrapper" id="item-image"></div>
                        <div class="qr-code-section">
                            <div id="item-qrcode-container"></div>
                            <button id="download-qr-btn" class="btn btn-sm btn-secondary"><i class="fa fa-download"></i> ${__("Download QR")}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        if (reader.is(":visible")) {
            if (html5QrCode && html5QrCode.isScanning) {
                html5QrCode.stop().then(() => {
                    reader.hide();
                    button.html(`<i class="fa fa-camera"></i> ${__("Scan with Camera")}`).removeClass('btn-danger').addClass('btn-primary');
                    $manualInputContainer.show();
                }).catch(err => console.error("Failed to stop camera.", err));
            }
        } else {
            reader.show();
            button.html(`<i class="fa fa-times"></i> ${__("Stop Camera")}`).removeClass('btn-primary').addClass('btn-danger');
            $manualInputContainer.hide();
            html5QrCode = new Html5Qrcode("qr-camera-reader", true);
            const qrCodeSuccessCallback = (decodedText, decodedResult) => {
                ...
                fetchItemData(decodedText, page);
            };
            const config = {
                fps: 10,
                qrbox: {
            ...
            html5QrCode.start({
                ...
                .catch(err => {
                    frappe.msgprint({
                        ...
                        message: __("Could not start camera. Please grant permission and ensure you are on a secure (https) connection."),
                        indicator: 'red'
                    });
                    button.html(`<i class="fa fa-camera"></i> ${__("Scan with Camera")}`).removeClass('btn-danger').addClass('btn-primary');
                    reader.hide();
...
function fetchItemData(scannedValue, page) {
    ...
    frappe.call({
        method: 'saturn.saturn.page.legal_stock_quantities.legal_stock_quantities.get_item_details_and_stock',
        args: {
        ...
        callback: function(r) {
            if (r.message) {
                ...
                populateStockLevels(r.message.stock_levels, r.message.details.legal_quantity);
                $detailsSection.show();
            } else {
                const error_msg = frappe.format(__("Item with barcode '{0}' not found or is incorrect."), [scannedValue]);
                $error.html(`<h5><i class="fa fa-exclamation-triangle"></i> ${__("Item Not Found")}</h5><p>${error_msg}</p>`).show();
                $input.val('');
            }
        },
        error: function(r) {
            $error.html(`<h5>${__("Server Error")}</h5><p>${__("An unexpected error occurred.")}</p>`).show();
            $detailsSection.hide();
        },
        always: function() {
            $loading.hide();
...
function generateQRCode(item_code, item_name) {
    const container = document.getElementById('item-qrcode-container');
    container.innerHTML = '';
    ...
    $('#download-qr-btn').off('click').on('click', function() {
        ...
        if (qrCanvas) {
            ...
            link.download = `QR-${item_name.replace(/[\s/\\?%*:|"<>]/g, '_')}.png`;
